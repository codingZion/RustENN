...
\begin{itemize}
    ...
    \item \mintinline{rust}{activation_function(x: f64) -> f64}, which computes the neuron activations described in~\ref{subsec:feed-forward-neural-networks-(fnns)}.
...
The EC process (in population.rs) starts with an initial population of new agents with NNs created by the \mintinline{rust}{NeuralNetwork::new()} function (described in~\ref{subsec:neural-network}).
...
\end{minted}
The NNs did not yet fully learn this task, however, modifying the ENN parameters might help the learning process.
