...
This is the easiest problem that can still be considered a game - with one match on the stack there is only one legal move, so it does not involve any decision-making.
...
It does not make sense for the NNs to have such complexity for such a simple problem as it only increases the needed computations.
...
Such local minima originate in early generations where the agents are not highly developed, which means they often only play one move.
