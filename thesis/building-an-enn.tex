...
\end{minted}
The vector inside (\mintinline{}{Vec<Node>}) represents a layer of the NN and the outside vector (\mintinline{}{Vec<Vec<Node>>}) contains all layers of the NN\@.
...
\begin{itemize}
    \item \mintinline{}{new(input_nodes: usize, output_nodes: usize) -> Self {}}, which initializes the NN with all input and output nodes connected to each other with random weights and biases.
    \item \mintinline{}{predict(&self, input: Vec<f64>) -> Vec<f64>}, which computes the output of the NN for a given input with the forward propagation algorithm described in~\ref{subsec:feed-forward-neural-networks-(fnns)}.
    \item \mintinline{}{activation_function(x: f64) -> f64}, which computes the neuron activations described in~\ref{subsec:feed-forward-neural-networks-(fnns)}.
...
\end{minted}
The function \mintinline{}{mutate(&mut self, mutations: usize) -> Self {} } has a number of mutations as a parameter which it applies to the NN of the agent.
...
The EC process (in population.rs) starts with an initial population of new agents with NNs created by the \mintinline{}{NeuralNetwork::new()} function (described in~\ref{subsec:neural-network}).
...
Since the stack size with 2 has already worked fine, the same configuration is run with an increased initial stack size of 8 matches:
\\
...
\begin{center}
    ...
    \end{center}
\end{center}
In this case the accuracy does not reach 100\% but stays around 25\% instead.
...
\begin{center}
    ...
    \end{center}
\end{center}
The NNs develop an average amount of over 20 hidden layers with 2 nodes on average.
It does not make sense for the NNs to have this complexity for such a simple problem, as it only increases the needed computations.
The chance for new nodes and edges to form is therefore reduced by a factor of 20 each.
The same configuration as before is run with the new parameter adjustments:
\\
...
\begin{center}
    ...
    \end{center}
\end{center}
The NNs reach full accuracy in less than 30 generations in both the first and second test run.
However, the NNs get stuck in a local minimum in the third run.
This is why additional test runs have been conducted with different parameter configurations.
...
Subsequently, the maximal amount of matches per stack is doubled which allows for 24 possible game states:
...
The NNs settle in all tests at an accuracy of around 57\%.
...
In the following tests, the same ENN parameter modifications as in the configuration Stack: 1--8, Configuration 2--5 are tested:
\begin{figure}[H]
    \centering
    \pgfplotsset{width=8.5cm,height=5cm,compat=1.18}
    % First graph
