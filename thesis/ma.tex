...
\addlegendentry{\(Sigmoid\)}
    ...
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    The accuracy of the best run got up to 70\% and if we look at the games recorded in generation 982, we can see why it didn't get to 100\%.
    \begin{minted}{}
    Generation: 982
    Best agent layer sizes: [1, 3, 1, 5, 6, 3, 2, 7, 3, 5, 3, 5, 1, 2, 8, 4, 7, 1, 2, 4, 4, 1, 9]
    Agent 39 vs Agent 982:
    Turn: 0: state: [8], agent_move: [0, 3]
    Turn: 1: state: [5], agent_move: [0, 5]
    Game result: [1, 0]
    Agent 982 vs Agent 39:
    Turn: 0: state: [8], agent_move: [0, 7]
    Turn: 1: state: [1], agent_move: [0, 1]
    Game result: [1, 0]
    Agent 981 vs Agent 982:
    Turn: 0: state: [8], agent_move: [0, 7]
    Turn: 1: state: [1], agent_move: [0, 5]
    Game result: [1, 0]
    Agent 982 vs Agent 981:
    Turn: 0: state: [8], agent_move: [0, 7]
    Turn: 1: state: [1], agent_move: [0, 1]
    Game result: [1, 0]
    \end{minted}
    The best agents of generation 982 and 981 each learned to play the correct move [0, 7] when they started.
    However, if the opponent didn't play perfectly, which means not removing less matches, the current best agent would have got a winning position but often didn't find the correct move.
    An example for this is on line 5, where the best agent from generation 982 should have removed 4 instead of 5 matches to win.
    \\
    Our final goal is to train the NNs to be adapted to all cases whereas this solution seems more like a local minimum which is therefore undesirable.
    ...
    We will therefore reduce the chance for a new nodes and edges to form by a factor of 10 each.
