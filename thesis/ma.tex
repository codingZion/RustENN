...
\begin{document}
    ...
    \end{mydef}
    \\ \\
    The computation of information by a neural network can be illustrated through the example of a computer vision NN that recognizes digits in a black-and-white image.
    As a first step, the input is encoded into values that correspond to the nodes of the input layer.
    In the example of a computer vision NN, the Brightness values of the single pixels can be used and directly assigned to the nodes of the input layer.
    \\
    Then, each node of the input layer is iterated through to identify all its connected edges.
