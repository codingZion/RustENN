...
\addlegendentry{\(Sigmoid\)}
    ...
    \\ \\
    The backpropagation algorithm is an algorithm that can find parameters for weights and biases of a NN with fixed topology.
    It uses a training adn test set containing the problems that the NN needs to solve as well as the corresponding solutions.
    First, it starts with a NN with fixed topology and random weights/biases.
    ...
    Its main idea is to have genes, that only encode indirectly for the parameters, and that can be on and off.
        ...
        \section{Evolutionary Neural Networks (ENNs) and the NEAT Approach}
