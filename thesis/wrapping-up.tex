...
\begin{itemize}
    \item \textbf{Rust} as the programming language.\cite{rust23}
    \item \textbf{Rust Crates}, as already described in section~\ref{sec:algorithm-design}~\cite{rand2024, indicatif2023, csv2023, bincode2021, serde2024, rayon2023}
    \item \textbf{JetBrains RustRover} as the integrated development environment.\cite{rustrover}
    \item \textbf{Git} as the version control system\cite{git}.
    \item \textbf{GitHub} as the online host of the repository\cite{github}.
    \item \textbf{GitHub Copilot} as the AI code completion tool to accelerate programming.\cite{github_copilot2021}
    \item \textbf{LaTex} as the typesetting system for the thesis.\cite{latex}
    \item \textbf{OpenAI ChatGPT} to enhance the language for some pre-written text parts and help troubleshoot technical problems.\cite{chatgpt}
    \item \textbf{Phoenix GUI} as a graphing tool to visualize the results of the tests.\cite{phoenixgui}
\end{itemize}
A big thank goes also to Gerd Altmann, for providing the cover image of this thesis.\cite{pixabay_neuronen}
