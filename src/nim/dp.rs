...
pub fn get_index(nim_heap_state: Vec<usize>, nim_heaps: Vec<usize>, bases: &Vec<usize>) -> usize {
    let mut index = 0;
...
pub fn dp_alg(nim_heap_state: Vec<usize>, nim_heaps: Vec<usize>, dp: &mut Vec<(i32, isize, bool)>, bases: &Vec<usize>) -> (i32, isize, bool) {
    let cur_index = get_index(nim_heap_state.clone(), nim_heaps.clone(), bases);
    if dp[cur_index].0 != -1 {
        return dp[cur_index];
    }
    ...
    for i in nim_heap_state.iter().enumerate() {
        if i.1.clone() != 0usize {
            for j in 1..i.1 + 1 {
                ...
                new_res.0 += 1;
                new_res.1 = get_index(new_state.clone(), nim_heaps.clone(), bases) as isize;
                new_res.2 = !new_res.2;
                if res.0 == -1 {
                    res = new_res;
                } else if new_res.2 {
